...
namespace HebrewTaskbarWidget.Services
{
    ...
    public static class UpdateService
    {
        ...
        /// <summary>
        /// true אם הבדיקה האחרונה (CheckForUpdateAsync) נכשלה ספציפית עקב
        /// היעדר חיבור לרשת (או המחשב כלל לא מחובר לרשת) - להבדיל מכשל אחר
        /// (למשל GitHub עצמו לא זמין זמנית, תגובה לא תקינה וכו'), כדי שממשק
        /// המשתמש יוכל להציג "אין חיבור לרשת" באופן מדוייק, במקום להשתמש
        /// בברירת המחדל המטעה "התוכנה מעודכנת לגרסה האחרונה".
        /// </summary>
        public static bool LastCheckWasNetworkFailure { get; private set; }
        ...
        public static async Task<UpdateInfo?> CheckForUpdateAsync()
        {
            ...
            LastCheckWasNetworkFailure = false;

            // בדיקה מקדימה וזולה: אם המחשב כלל לא מחובר לרשת (לא רק "אין
            // אינטרנט יוצא", אלא גם למשל כבל/Wi-Fi מנותקים לגמרי) - אין טעם
            // אפילו לנסות את הבקשה ל-GitHub, וממילא נדע בוודאות שזו הסיבה.
            if (!NetworkInterface.GetIsNetworkAvailable())
            {
                LastCheckWasNetworkFailure = true;
            ...
            {
                // כשל תקשורת קלאסי (אין חיבור בפועל, נכשל DNS/TLS, השרת לא
                // הגיב וכו') - נחשב "אין חיבור לרשת" מבחינת המשתמש.
                LastCheckWasNetworkFailure = true;
                return null;
            }
            catch (TaskCanceledException)
            {
                // Timeout על הבקשה (ראו Client.Timeout למעלה) - כמעט תמיד
                // סימן לבעיית רשת/חיבור איטי מדי, לא לבעיה אצל GitHub עצמו.
                LastCheckWasNetworkFailure = true;
                return null;
            }
            catch
            {
                // כשל שקט אחר (למשל JSON לא תקין) - לא בהכרח בעיית רשת, אז
                // אין להטעות ולהציג "אין חיבור לרשת". הבדיקה הבאה תנסה שוב
                // (עד 24 שעות מהניסיון הזה).
                return null;
            }
        }

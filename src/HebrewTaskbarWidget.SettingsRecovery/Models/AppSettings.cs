...
namespace HebrewTaskbarWidget.Models
{
    ...
    public sealed class AppSettings
    {
        ...
        // --- סיור היכרות ראשוני (OnboardingTourWindow) ---
        /// <summary>
        /// false (ברירת המחדל) - הסיור עדיין לא "נסגר לצמיתות" ע"י המשתמש,
        /// ולכן יוצג אוטומטית בכל עליית תוכנה (ראו MainWindow_ContentRendered).
        /// עובר ל-true רק בלחיצה מפורשת על "דלג" בחלונית הסיור עצמה - לחיצה
        /// על "הזכר לי אחר כך" משאירה את זה false בכוונה, כדי שהסיור יוצג
        /// שוב בפעם הבאה שהתוכנה עולה. ניתן להריץ את הסיור בכל עת גם אחרי
        /// שדולג עליו, דרך כפתור ייעודי בלשונית "כללי" - זה לא נוגע בערך הזה.
        /// </summary>
        public bool OnboardingTourSkipped { get; set; } = false;

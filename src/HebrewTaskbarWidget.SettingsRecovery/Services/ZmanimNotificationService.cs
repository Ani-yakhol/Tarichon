                    ...
                    if (now >= notifyAt && now <= entry.Time.Value)
                    {
                        ...
                        if (settings.NotificationShowPopup)
                        {
                            ShowToast(ZmanimCalendar.GetPopupDisplayName(entry), entry.Time.Value, minutesBefore, onSnoozeReplaySound: () =>
                                NotificationSoundService.PlayForZman(settings, rule.SoundOverridePath, rule.SoundOverrideFixedName, rule.SoundOverrideUseVoice, entry.VoiceKey, (int)minutesBefore, entry.Time));
                        }

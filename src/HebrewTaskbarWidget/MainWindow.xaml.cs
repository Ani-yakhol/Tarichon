...
namespace HebrewTaskbarWidget
{
    public partial class MainWindow : Window
    {
        ...
        // תוצאת הבדיקה "האם הלחיצה השמאלית הנוכחית היא על חצי השעון/תאריך
        // הלועזי" - נקבעת בזמן הלחיצה (WM_LBUTTONDOWN) ונצרכת בזמן השחרור
        // (WM_LBUTTONUP), ראו OnRawLeftButtonDown/OnRawLeftButtonUp.
        private bool _pendingLeftClickIsGregorianForward;
        private IntPtr _pendingGregorianClockWnd;
        private RECT _pendingGregorianClockRect;
        ...
        public MainWindow()
        {
            ...
            // הוידג'ט עצמו (כל 150 מ"ש) - זו הייתה הסיבה לכך שהתפריט (שאינו
            // Topmost בעצמו) יכול "להיבלע" מתחת לוידג'ט בין אכיפה לאכיפה.
            ...
            // העצמאי לפאנל ההגדרות, HebrewTaskbarWidgetSettings.exe) - כדי
        ...
        private void OnRawLeftButtonDown(IntPtr hwnd, IntPtr lParam)
        {
            ...
            else
            {
                ...
                // בודקים כבר עכשיו (בזמן הלחיצה, לא בזמן השחרור) על איזה חצי
                // מהוידג'ט לחצו - כדי שבשחרור נדע אם לפתוח את לוח הזמנים שלנו
                // (ברירת המחדל - כל לחיצה, כולל על חצי השעון/תאריך הלועזי)
                // או להעביר את הלחיצה לשעון האמיתי של Windows כדי לפתוח את
                // לוח השנה המובנה שלו (רק אם ההגדרה הרלוונטית הופעלה במפורש,
                // וגם רק אם ממש לחצו על חצי השעון/תאריך הלועזי).
                _pendingLeftClickIsGregorianForward = SettingsService.Current.OpenWindowsCalendarOnGregorianClick &&
                    TryGetGregorianClockForwardTarget(hwnd, lParam, out _pendingGregorianClockWnd, out _pendingGregorianClockRect);
            }
        }
        ...
        private void OnRawLeftButtonUp()
        {
            ...
            if (_leftButtonDownPending)
            {
                ...
                if (_pendingLeftClickIsGregorianForward)
                {
                    ForwardLeftClickToWindowsClock(_pendingGregorianClockWnd, _pendingGregorianClockRect);
                }
                else
                {
                    ToggleZmanimPopup();
        ...
        /// <summary>
        /// מדמה לחיצה שמאלית **אמיתית** (SendInput) במרכז מלבן השעון האמיתי
        /// של Windows, כדי לפתוח את לוח השנה/החלונית המובנית שנפתחת בלחיצה
        /// שמאלית על השעון המקורי בשורת המשימות - אותו מנגנון בדיוק כמו
        /// <see cref="ForwardRightClickToWindowsClock"/>, רק עם כפתור שמאלי.
        /// </summary>
        private static void ForwardLeftClickToWindowsClock(IntPtr clockWnd, RECT clockScreenRect)
        {
            if (!NativeMethods.IsWindowVisible(clockWnd))
            {
                NativeMethods.ShowWindow(clockWnd, NativeMethods.SW_SHOW);
            ...
            var inputs = new NativeMethods.INPUT[2];
            inputs[0] = new NativeMethods.INPUT { type = NativeMethods.INPUT_MOUSE, mi = new NativeMethods.MOUSEINPUT { dwFlags = NativeMethods.MOUSEEVENTF_LEFTDOWN } };
            inputs[1] = new NativeMethods.INPUT { type = NativeMethods.INPUT_MOUSE, mi = new NativeMethods.MOUSEINPUT { dwFlags = NativeMethods.MOUSEEVENTF_LEFTUP } };
            NativeMethods.SendInput(2, inputs, System.Runtime.InteropServices.Marshal.SizeOf<NativeMethods.INPUT>());
        ...
        private void UpdateContent()
        {
            ...
            // שעון/תאריך לועזי - מחקה את הפורמט של השעון המקורי של Windows, לפי
            // הגדרות אזוריות (Region) נוכחיות של המשתמש, בדיוק כמו שהשעון עצמו עושה.
            string gregorianTime = AppTimeService.FormatClockTime(now);
            string gregorianDate = now.ToString("d", CultureInfo.CurrentCulture);
                ...
                !StartupService.IsAutoStartLaunch() && WindowsClockVisibilityService.NeedsExplorerRestart())
            {
                switch (SettingsService.Current.ExplorerAutoLaunchMode)
                {
                    ...
                    case ExplorerAutoLaunchMode.Never:
                        // המשתמש ביקש לעולם לא לשאול/להפעיל אוטומטית - אם
                        // יידרש, יעשה זאת ידנית דרך המתג בפאנל ההגדרות.
                        break;
        ...
        private void UpdatePosition()
        {
            ...
            if (Math.Abs(Left - newLeft) > 0.5 || double.IsNaN(Left))
            {
                Left = newLeft;
            }

            if (Math.Abs(Top - newTop) > 0.5 || double.IsNaN(Top))
            {
                Top = newTop;
            }
        }
        ...
        private void ShutDownCompletely()
        {
            ...
            // סגירת התוכנה, עד הפעלה מחדש נוספת של Explorer. שואלים את
            // המשתמש אם לבצע את זה עכשיו (ולא אוטומטית, כי זה גורם להבהוב
            // קצר של שולחן העבודה).
            if (SettingsService.Current.HideWindowsClock && SettingsService.Current.HideWindowsClockReduceGap)
            {

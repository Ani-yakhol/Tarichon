...
namespace HebrewTaskbarWidget.Services
{
    ...
    public static class ZmanimNotificationService
    {
        ...
        private static void CheckAndNotify()
        {
            ...
            IReadOnlyList<ZmanEntry> todaysZmanim = ZmanimCalendar.Calculate(today, location);
            ...
            if (settings.NotificationShowPopup || settings.NotificationPlaySound)
            {
                foreach (ZmanNotificationRule rule in settings.ZmanNotificationRules)
                {
                    ...
                    if (now >= notifyAt && now <= entry.Time.Value)
                    {
                        ...
                        {
                            ShowToast(entry.Name, entry.Time.Value, minutesBefore);
                        }
        ...
        private static void ShowToast(string zmanName, DateTime zmanTime, double minutesBefore, double? durationSecondsOverride = null, bool? darkBackgroundOverride = null)
        {
            string timeText = AppTimeService.FormatZmanTime(zmanTime);
            ToastNotificationWindow.Show(zmanName, minutesBefore, timeText, isTest: false, durationSecondsOverride, darkBackgroundOverride, zmanTime);
        }
    }
}
